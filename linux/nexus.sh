...
trap 'echo "An error occurred. Exiting..." >&2; exit 1'
set -e
...
if [ -d "/opt/nexus/nexus" ]; then
  echo "[WARN] Nexus already installed at /opt/nexus/nexus. Aborting."
  exit 1
fi
...
sudo cat /opt/nexus/sonatype-work/nexus3/admin.password || echo "[WARN] Password file not found yet. Nexus may still be starting."
